...
func Get() *gin.Engine {
	...
	org := api.Group("/org")
	{
		...
		org.PUT("/:id/admin", s.mustLogin, s.hasPermission([]int{entity.AuthManageOrg}), s.updateOrgById)
		org.PUT("/api/org/self", s.mustLogin, s.hasPermission([]int{entity.AuthListOrgOrder}), s.updateSelfOrgById)
	}
	orgs := api.Group("/orgs")
	{
